...
/* Простая функция освобождения (для тестов) */
static void free_test_config(level_config_t *config) {
    if (!config) return;
    free(config->objects);
    if (config->layout) {
...
/* Тест: создание конфигурации по умолчанию */

START_TEST(test_create_default_config) {
    level_config_t *cfg = CreateDefaultConfig();
    ck_assert_ptr_nonnull(cfg);

    ck_assert_str_eq(cfg->name, "Default");
    ck_assert_str_eq(cfg->next_level, "");
    ck_assert_str_eq(cfg->texture_background, "resources/backgrounds/default_bg.png");

    ck_assert_ptr_null(cfg->objects);
    ck_assert_int_eq(cfg->count_objects, 0);

    ck_assert_ptr_nonnull(cfg->layout);
    ck_assert_int_eq(cfg->layout->rows, 500);
    ck_assert_int_eq(cfg->layout->cols, 500);
    ck_assert_ptr_nonnull(cfg->layout->data);

    ck_assert_int_eq(cfg->layout->data[0][0], 0);

    free_test_config(cfg);
}
END_TEST

START_TEST(test_expand_map_to_editor_size) {
    level_config_t *cfg = malloc(sizeof(level_config_t));
    memset(cfg, 0, sizeof(level_config_t));
    cfg->layout = malloc(sizeof(map_layout_t));
    cfg->layout->rows = 3;
    cfg->layout->cols = 4;
    cfg->layout->data = malloc(sizeof(int*) * 3);
    for (int i = 0; i < 3; i++) {
        cfg->layout->data[i] = malloc(sizeof(int) * 4);
        for (int j = 0; j < 4; j++) {
            cfg->layout->data[i][j] = (i + j) % 3;
        }
    }

    ExpandMapToEditorSize(cfg, 500, 500);
    ...
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 4; j++) {
            int expected = (i + j) % 3;
            ck_assert_int_eq(cfg->layout->data[i][j], expected);
        }
    }

    ck_assert_int_eq(cfg->layout->data[0][4], 0);
    ck_assert_int_eq(cfg->layout->data[3][0], 0);
    ck_assert_int_eq(cfg->layout->data[5][5], 0);
...
Suite *map_editor_suite(void) {
    Suite *s = suite_create("MapEditor");
    TCase *tc = tcase_create("Core");
    tcase_add_test(tc, test_create_default_config);
    tcase_add_test(tc, test_expand_map_to_editor_size);
...
int main(void) {
    int number_failed;
    SRunner *sr = srunner_create(map_editor_suite());
