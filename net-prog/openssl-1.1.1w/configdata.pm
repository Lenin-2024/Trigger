...
our %config = (
  ...
  openssldir => "/usr/riscv64-linux-gnu/ssl",
  options => "--cross-compile-prefix=riscv64-linux-gnu- --prefix=/usr/riscv64-linux-gnu --openssldir=/usr/riscv64-linux-gnu/ssl no-afalgeng no-asan no-buildtest-c++ no-crypto-mdebug no-crypto-mdebug-backtrace no-devcryptoeng no-ec_nistp_64_gcc_128 no-egd no-external-tests no-fuzz-afl no-fuzz-libfuzzer no-heartbeats no-md2 no-msan no-rc5 no-sctp no-ssl-trace no-ssl3 no-ssl3-method no-ubsan no-unit-test no-weak-ssl-ciphers no-zlib no-zlib-dynamic",
  perl_archname => "x86_64-linux-gnu-thread-multi",
  perl_cmd => "/usr/bin/perl",
  perl_version => "5.38.2",
  perlargv => [ "linux64-riscv64", "--cross-compile-prefix=riscv64-linux-gnu-", "--prefix=/usr/riscv64-linux-gnu", "--openssldir=/usr/riscv64-linux-gnu/ssl" ],
  perlenv => {
      "AR" => undef,
      "ARFLAGS" => undef,
      "AS" => undef,
